...
```
Notice that I stored the result in a vector called ``sims``. When calculating probabilities, we're not interested in *each* of the outcomes, but their *relative frequencies*. Using the ``table`` function, we can summarize the result as follows:
```{r}
table(sims)
```
